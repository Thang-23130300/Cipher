...
<div class="product-type-dynamic" id="productTypeResults" aria-live="polite">
    <div class="category-header">
        <h2><c:out value="${data.category.categoryName}"/></h2>
    ...
    <c:if test="${hasActiveFilter}">
        <div class="active-filter-bar">
            ...
            <c:if test="${not empty data.minPrice or not empty data.maxPrice}">
                <strong>
                    <c:choose>
                        <c:when test="${empty data.minPrice}">Dưới <fmt:formatNumber value="${data.maxPrice}" groupingUsed="true"/>đ</c:when>
                        <c:when test="${empty data.maxPrice}">Trên <fmt:formatNumber value="${data.minPrice}" groupingUsed="true"/>đ</c:when>
                        <c:otherwise><fmt:formatNumber value="${data.minPrice}" groupingUsed="true"/>đ - <fmt:formatNumber value="${data.maxPrice}" groupingUsed="true"/>đ</c:otherwise>
                    </c:choose>
                </strong>
            </c:if>
            <c:if test="${not empty data.rating}"><strong>Từ ${data.rating} sao</strong></c:if>
            <c:if test="${data.sortParam != 'popular'}">
                <strong>${data.sortParam == 'newest' ? 'Mới nhất' : (data.sortParam == 'price_asc' ? 'Giá thấp đến cao' : 'Giá cao đến thấp')}</strong>
            </c:if>
    ...
    <c:if test="${data.totalPages > 1}">
        <div class="pagination">
            <c:forEach begin="1" end="${data.totalPages}" var="i">
                ...
                <a href="${pageUrl}" class="${i == data.currentPage ? 'active' : ''}">${i}</a>
            </c:forEach>
        </div>
    </c:if>
</div>
