...
public class ProductTypeService {
    ...
    public ProductTypeDTO getSearchProductType(
            String keyword,
            Integer minPrice,
            Integer maxPrice,
            Integer rating,
        ...
        int totalProducts = productDAO.countSearchProductsWithFilter(
                safeKeyword, minPrice, maxPrice, rating
        );
        ...
        List<Product> products = productDAO.searchProductsWithFilter(
                safeKeyword, minPrice, maxPrice, rating, sort, offset, PAGE_SIZE
        );
    ...
    public ProductTypeDTO getPanelProductType(
            String panelSlug,
            Integer minPrice,
            Integer maxPrice,
            Integer rating,
        ...
        int totalProducts = productDAO.countProductsByCategoryIdsWithFilter(
                panel.categoryIds(), minPrice, maxPrice, rating
        );
        int totalPages = Math.max(1, (int) Math.ceil((double) totalProducts / PAGE_SIZE));
        int safePage = Math.min(Math.max(page, 1), totalPages);
        int offset = (safePage - 1) * PAGE_SIZE;
        ...
        List<Product> products = productDAO.getProductsByCategoryIdsWithFilter(
                panel.categoryIds(), minPrice, maxPrice, rating, sort, offset, PAGE_SIZE
        );
        ...
        private static PanelDefinition from(String slug) {
            ...
            return switch (slug.trim()) {
                case "vi-que-binh-dinh" -> new PanelDefinition(
                        "vi-que-binh-dinh", "Vị Quê Bình Định", List.of(1L, 2L)
                );
                case "qua-tu-bien" -> new PanelDefinition(
                        "qua-tu-bien", "Quà Từ Biển", List.of(4L)
                );
                case "huong-men-dat-vo" -> new PanelDefinition(
                        "huong-men-dat-vo", "Hương Men Đất Võ", List.of(3L)
                );
                case "net-viet-lam-qua" -> new PanelDefinition(
                        "net-viet-lam-qua", "Nét Việt Làm Quà", List.of(5L, 6L)
                );
                default -> null;
            };
        }
